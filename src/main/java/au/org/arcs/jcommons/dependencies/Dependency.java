...
//	CLIENTSIDEMDS("https://code.arcs.org.au/hudson/job/Grisu-SNAPSHOT-binaries/lastSuccessfulBuild/artifact/frontend/client-side-mds/target/client-side-mds.jar"),
	ARCSGSI("${repository}/au/org/arcs/auth/arcs-gsi/${version}", "arcs-gsi-${version}-lib.jar"),
	ARCSGSI_WITHOUTPYTHON("${repository}/au/org/arcs/auth/arcs-gsi/${version}", "arcs-gsi-${version}-lib-without-jython.jar"),
	BOUNCYCASTLE("http://www.bouncycastle.org/download/", "bcprov-${version}.jar"), 
	GRISU_LOCAL_BACKEND("${repository}/org/vpac/grisu/grisu-core/${version}", "grisu-core-${version}-backend.jar"), 
	GRISU_XFIRE_CLIENT_LIBS("$repository}/org/vpac/grisu/grisu-client-xfire/${version}", "grisu-client-xfire-${version}.jar"),
	CLIENT_SIDE_MDS("${repository}/org/vpac/grisu/client-side-mds/${version}", "client-side-mds-${version}-side-mds.jar");
	...
	public String getDownloadUrl(String version) {
		...
		String temp = this.downloadUrl.replace("${version}", version) + "/" + getFileName(version);
