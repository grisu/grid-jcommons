...
//	}
	...
	public static void setHttpProxy(String proxyHost, int proxyPort, String username, char[] password) {
		setHttpProxy(proxyHost, proxyPort);
		if ( StringUtils.isNotBlank(username) ) {
			setHttpAuth(username, password);
			// for jython
			System.getProperties().put( "http_proxy", "http://"+username+":"+new String(password)+"@"+proxyHost+":"+proxyPort+"/" );
			System.getProperties().put( "https_proxy", "http://"+username+":"+new String(password)+"@"+proxyHost+":"+proxyPort+"/" );
		} else {
			// for jython
			System.getProperties().put( "http_proxy", "http://"+proxyHost+":"+proxyPort+"/" );
			System.getProperties().put( "https_proxy", "http://"+proxyHost+":"+proxyPort+"/" );
			CommonArcsProperties.getDefault().setArcsProperty(CommonArcsProperties.Property.HTTP_PROXY_USERNAME, "");	

		}
		
		EventBus.publish(new NewHttpProxyEvent(proxyHost, proxyPort, username, password));
	}
	...
	private static void setHttpProxy(String proxyHost, int proxyPort) {
		...
		if ( StringUtils.isBlank(proxyHost) ) {
			...
			CommonArcsProperties.getDefault().setArcsProperty(CommonArcsProperties.Property.HTTP_PROXY_HOST, "");
			CommonArcsProperties.getDefault().setArcsProperty(CommonArcsProperties.Property.HTTP_PROXY_PORT, "");
		} else {
