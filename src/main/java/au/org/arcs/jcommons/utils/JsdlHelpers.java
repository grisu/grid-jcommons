...
public final class JsdlHelpers {
	 ...
	 *            the root url for the filesystem
	 * @return the filesystem element
	 */
	public static Element addOrRetrieveExistingFileSystemElement(
	...
	/**
	 * Parses the jsdl document and returns the value of the Description
	 * element.
	 * 
	 * @param jsdl
	 *            the jsdl document
	...
	// */
	// public static void addCandidateHosts(final Document jsdl, final String[]
	// subLocs)
	// throws XPathExpressionException {
	// String expression =
	// "/jsdl:JobDefinition/jsdl:JobDescription/jsdl:Resources/jsdl:CandidateHosts";
	// NodeList resultNodes = null;
	// try {
	// resultNodes = (NodeList) xpath.evaluate(expression, jsdl,
	// XPathConstants.NODESET);
	 ...
	 */
	public static Element getStageInTarget_filesystemPart(
	 ...
	 */
	public static Element getStageInTarget_relativePart(
			final Element stageInElement) {
	 ...
	 */
	public static Map<String, String> getTemplateTagInfoItems(
			final Document jsdl, final String templateTagName) {
	 ...
	 */
	public static void setApplicationVersion(final Document jsdl,
	 ...
	 *             if the candidatehosts element could not be found
	 */
	public static void setCandidateHosts(final Document jsdl,
			final String[] subLocs) {
		...
		for (String subLoc : subLocs) {
			hostName = jsdl.createElementNS(nsURL, "HostName");
			hostName.setTextContent(subLoc);
			resultNodes.item(0).replaceChild(hostName,
					resultNodes.item(0).getFirstChild());
		}
	}
	...
	/**
	 * Sets the totalCPUcount of the job. If there is already a cpucount
	 * specified it will be overwritten.
	 * 
	 * @param jsdl
	 *            the jsdl document
	 ...
	 *             if the JobName element could not be found
	 */
	public static void setProcessorCount(final Document jsdl,
		...
		if (resultNodes.getLength() > 1) {
			...
			// replace the text content of the already existing JobName element
			resultNodes.item(0).setTextContent(
					new Integer(processorcount).toString());
		}
	...
	/**
	 * Sets the totalCPUTime of the job in seconds. If there is already a
	 * walltime specified it will be overwritten.
	 * 
	 * Be aware that you need to multiply the walltime in seconds times the
	 * number of cpus for this job to get the proper totalcputime.
	 * 
	 * @param jsdl
	 *            the jsdl document
	 * @param totalcputimeinseconds
	 ...
	 *             if the JobName element could not be found
	 */
	public static void setTotalCPUTimeInSeconds(final Document jsdl,
			final int totalcputimeinseconds) {
		String expression = "/jsdl:JobDefinition/jsdl:JobDescription/jsdl:Application/jsdl:TotalCPUTime";
		NodeList resultNodes = null;
		try {
			resultNodes = (NodeList) xpath.evaluate(expression, jsdl,
		...
		if (resultNodes.getLength() > 1) {
			...
			// replace the text content of the already existing JobName element
			resultNodes.item(0).setTextContent(
					new Integer(totalcputimeinseconds).toString());
		}
